...
public class QuestionPages {
	...
	public void readData() {
		...
		ResultSet rs = db.Read("SELECT * from \"OODQuiz1Questions\"");
		try {
			...
			while (rs.next()) {
				for (int i = 1; i <= rsm.getColumnCount(); i++) {
					if (i == 1) {
						...
						questions.get(questions.size() - 1).cAnswers = rs.getString(i);
						...
						for (int j = 0; j < rs.getString(i).length(); j++) {
							if (rs.getString(i).charAt(j) == ',') {
								questions.get(questions.size() - 1).answers.add(rs.getString(i).substring(k, j-1));
								k=j+2;
							}
							else if((j + 2) == rs.getString(i).length()) {
								questions.get(questions.size() - 1).answers.add(rs.getString(i).substring(k, j+1));
							}
						}
					}
